...
Wiki 投影会为实体页生成 `Current Conclusion`、`Recent Changes`、`High Confidence Facts`、`Evidence Excerpts` 等知识页结构；为主题页生成状态分组；为时间线页生成 `Event Flow`，并在 archive 事件存在时带上 `cause/process/result`。`lint_memory_wiki` 会检查这些结构是否缺失、是否仍停留在旧式泛化 Summary、以及图谱证据是否不足。
