...
```bash
cd ~/openclaw
openclaw plugins install cortex-memory-pro@latest
...
推荐使用一条命令更新到 npm 最新版，并在完成后重启 gateway：

```bash
npx -y -p cortex-memory-pro@latest cortex-memory update --restart
```

如果当前环境已经能直接运行 `cortex-memory`，也可以使用：
...
```bash
cd ~/openclaw
openclaw plugins disable cortex-memory-pro
openclaw plugins uninstall cortex-memory-pro
```
...
cd ~/openclaw
openclaw plugins disable cortex-memory-pro
openclaw plugins uninstall cortex-memory-pro --keep-data
```
...
- `plugins.slots.memory` 不要设置为 `cortex-memory-pro`
...
- Linux/macOS: `~/.openclaw/workspace/memory/cortex-memory-pro`
- Windows: `%USERPROFILE%\\.openclaw\\workspace\\memory\\cortex-memory-pro`
